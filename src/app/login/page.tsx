      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-8 bg-background">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {error && (
              <p className="text-sm text-error bg-error-container/40 px-4 py-3 rounded-xl">{error}</p>
            )}
          ...
          <p className="text-center text-on-surface-variant mt-8 text-sm">
            {t('login.noAccount')}{' '}
            <Link href="/register" className="text-primary font-bold hover:underline">{t('login.signUp')}</Link>
          </p>
        </div>
      </div>
    </div>
