...
export default function SettingsPage() {
  ...
  async function enablePush() {
    ...
    else { setPushStatus('error'); setPushErr(res.reason || 'Failed'); }
  }
                ...
                {[
                  ...
                  <div key={row.label} className="flex items-center gap-4 px-6 py-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-semibold text-on-surface text-sm truncate">{row.val || '—'}</p>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  {kids.map((k: any) => {
                    ...
                    return (
                      <Link key={k.student_id} href={`/kids/${k.student_id}`}
                        ...
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0 ${approved ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
              ...
              <div className="p-6 flex items-center gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-on-surface-variant mt-0.5">
                    {pushStatus === 'enabled' ? t('settings.pushEnabled')
                    : pushStatus === 'denied' ? t('settings.pushBlocked')
                    : pushStatus === 'unsupported' ? t('settings.pushUnsupported')
                    : !hasFirebaseConfig() ? t('settings.pushNotConfigured')
            ...
            <div className="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant/30 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <label className="flex items-center gap-2 text-xs text-on-surface-variant cursor-pointer select-none">
                  <input type="checkbox" checked={showPw} onChange={e => setShowPw(e.target.checked)} className="w-3.5 h-3.5 accent-primary" />
                  {t('settings.showPw')}
                </label>
